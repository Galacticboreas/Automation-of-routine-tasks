...
from app.db_models.orders import ReportMovingSetsOfFurniture, Report
...
def extract_data_to_report_moving_sets_of_furnuture(orders_data: dict,
    ...
    for value in tqdm(workbook_sheet.iter_rows(min_row=2, values_only=True), ncols=80, ascii=True, desc=sheet):
        ...
        if (expression in full_order_number) and (ordered is not None):
            order_report = ReportMovingSetsOfFurniture()
            order_report.full_order_number = full_order_number
            order_report.furniture_name = furniture_name
            order_report.furniture_atricle = Report.extractor.get_article(furniture_name)
            order_report.oredered = ordered
            order_report.released = released
            order_report.remains_to_release = remains_to_release
            orders_data[composite_key] = order_report
    return orders_data
