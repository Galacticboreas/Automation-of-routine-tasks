...
os.popen(), и к томму же цель этого модуля состоит в том, чтобы запустить
...
соответствующие участики программного кода в PyEdit и в других сценариях;
...
def fixWindowsPath(cmdline):
    ...
    return ' '.join([fixedpath] + splitline[1:])  # снова обхединить в строку
...
class LaunchMode:
    ...
    команда должна начаться с имени запускаемого файла сценария Python
    и не дожна начинаться со слова "python" или с полного пути к нему;
    """
    ...
    def run(self, cmdline):
        assert False, 'run munst be defined'
...
class Start(LaunchMode):
    ...
    только для Windows: использует ассоциации с расширениями имен фалов
    """
